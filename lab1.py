...
class Character(ABC)
	...
	def attack(self):
		return floor(randint(1,13)) + floor((self.strength - 10) / 2)

	def save_throw(self,attribute):
		return floor(randint(1,21)) + floor((attribute - 10) / 2)
...
class Hero(Character):
	def perk(self):
		return floor(randint(1,17)) + floor(((self.wisdom + self.hp) - 10) / 2)

class Dragon(Character):
	def perk(self):
		return self.hp -= floor(randint(1,17)) + floor((self.attack() / 2))